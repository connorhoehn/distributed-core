...
import { WALWriter, WALReader, WALConfig, EntityState as PersistenceEntityState } from '../../../persistence/types';
...
export class WriteAheadLogEntityRegistry extends EventEmitter implements EntityRegistry {
  ...
  async proposeEntity(entityId: string, metadata?: Record<string, any>): Promise<EntityRecord> {
    ...
    // Write to WAL first
    await this.walWriter.append(update).then(() => void 0);
  ...
  exportSnapshot(): EntitySnapshot {
    return {
      ...
      entities: Object.fromEntries(
        Array.from(this.entities.entries()).map(([entityId, record]) => [
          entityId,
          {
            ...record,
  ...
  async getCurrentLSN(): Promise<number> {
    return ((this.walWriter as unknown) as WALWriterImpl).getCurrentLSN();
  }
  ...
  async truncateWALBefore(lsn: number): Promise<void> {
    await ((this.walWriter as unknown) as WALWriterImpl).truncateBefore(lsn);
  }
    ...
    if (snapshot) {
      ...
      for (const [entityId, entityState] of Object.entries(snapshot.entities)) {
        const record: EntityRecord = {
          ...
          createdAt: entityState.createdAt ?? Date.now(),
          lastUpdated: entityState.lastModified ?? Date.now(),
          metadata: entityState.metadata || {}
        };
        this.entities.set(entityId, record);
      }
      
      this.lastCheckpointLSN = snapshot.lsn ?? 0;
  ...
  async createCheckpoint(): Promise<void> {
    ...
    const entities: Record<string, PersistenceEntityState> = {};
    for (const [entityId, record] of this.entities) {
      entities[entityId] = {
        id: record.entityId,
        state: record.metadata, // Map metadata to state
