...
export class InMemoryEntityRegistry extends EventEmitter implements EntityRegistry {
  ...
  exportSnapshot(): EntitySnapshot {
    // Convert EntityRecord to CheckpointEntityState for each entity
    const entities: Record<string, CheckpointEntityState> = {};
    for (const [entityId, record] of this.entities.entries()) {
      entities[entityId] = {
